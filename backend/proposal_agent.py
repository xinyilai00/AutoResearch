...
- IMPORTANT FOR REQUIRED DATA FILES: do not assume files such as data/.../*.txt or data/.../*.csv exist unless they appear in the README or file tree. If the repo requires external data, put the download/preparation commands in data_setup_commands. In run_script, verify required files with os.path.exists before calling repo utilities, print available data files when missing, and choose an included small sample file when appropriate.
- IMPORTANT ABOUT FILE PATHS: your script will always be executed with its working directory (cwd) already set to the root of the cloned repository. This is true on every execution environment this pipeline uses. Therefore: always reference repo files using relative paths (e.g. "data/file.csv", "models/DLinear.py") or os.getcwd(). NEVER use absolute filesystem paths (e.g. "/content/...", "/tmp/..."). NEVER search the filesystem to locate your own repo's files (e.g. do not use subprocess with "find", os.walk("/"), or similar). You already know the repo's structure from the file tree provided below — use it directly.
...
- IMPORTANT FOR PANDAS DATAFRAMES: never hardcode a DataFrame column name unless it appears in the README/file tree context or the script verifies it exists in df.columns first. For CSV/dataframe experiments, print available columns, build candidate column names, filter candidates by membership in df.columns, and fall back to an existing numeric/target column when appropriate. If the study needs progression/cycle/semester and no explicit column exists, infer progression from actual numbered stage/course columns present in df.columns rather than inventing names like Cycle or Ultimo Ciclo.
- IMPORTANT FOR CATEGORICAL FEATURES: before passing pandas DataFrames to sklearn, imblearn, SMOTE, regressors, or classifiers, encode non-numeric feature columns with pandas.get_dummies or a ColumnTransformer and align train/test encoded columns. Values such as Yes/No, gender, shift, specialty, or labels must not be passed directly into numeric estimators.
- IMPORTANT FOR RESAMPLING/SMOTE: only apply SMOTE/SMOTETomek/fit_resample after checking that the target vector has at least two classes and enough samples in each class. If a semester/subset has one target class, skip resampling for that subset and report the limitation.
